...
class DynamicHead(nn.Module):
    ...
    def __init__(self, cfg, roi_input_shape):
        ...
        self.rcnn_head = RCNNHead(cfg, d_model, num_classes, dim_feedforward, nhead, dropout, activation)
    ...
    def forward(self, features, init_bboxes, t, init_features, images_whwh, pos_imgs, pos_local):
...
class RCNNHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb, images_whwh, pos_imgs, pos_local):
        """
        ...
        bboxes_xywh = bboxes / images_whwh[:, None, :]
        bboxes_xywh = box_xyxy_to_cxcywh(bboxes_xywh)
        pos_xywh = gen_sineembed_for_position(bboxes_xywh)
        ...
        pro_features2 = self.inst_interact(pro_features, roi_features, pos_xywh, pos_img, pos_local, bboxes_xywh)
...
class DynamicConv(nn.Module):
    ...
    def forward(self, pro_features, roi_features, pos_xywh, pos_img, pos_local, bboxes_xywh):
        '''
        pro_features: (1,  N * nr_boxes=300, self.d_model=256)
        roi_features: (49, N * nr_boxes=300, self.d_model=256)
        pos_xywh：(1, 300, 512)bboxes_xywh的pe
        pos_img: (49,300,256)
        pos_local: (7,7)
        bboxes_xywh: (1,300,4)
        '''
