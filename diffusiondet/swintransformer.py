...
weight = "/home/shau/Desktop/DiffusionDet-main/models/swin_tiny_patch4_window7_224.pth"
config = size2config["T"]
out_indices = (0, 1, 2, 3)
model = SwinTransformer(
    embed_dim=config['embed_dim'],
    window_size=config['window_size'],
...
print(out["swin3"].size(), out["swin2"].size(), out["swin1"].size(), out["swin0"].size())
# torch.Size([2, 768, 20, 20]) torch.Size([2, 384, 40, 40]) torch.Size([2, 192, 80, 80]) torch.Size([2, 96, 160, 160])
in_features=["swin0", "swin1", "swin2", "swin3"]
for f in in_features:
    print(model.output_shape()[f].stride, model.output_shape()[f].channels)
    print(model.output_shape(), model.output_shape()[f].channels)
model1 = FPN(bottom_up=model,
             in_features=["swin0", "swin1", "swin2", "swin3"],
             out_channels=256,
             )
out1 = model1(torch.randn(2, 3, 640, 640))
print(out1["p2"].shape, out1["p5"].shape)
"""